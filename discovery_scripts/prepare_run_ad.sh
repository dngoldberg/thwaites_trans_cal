...
code_dir=code
input_dir=$HOME/thwaites_trans_cal/input_tc/
if [ $2 == 'snap' ]; then
...
ln -s $input_dir/* .
ln -s $HOME/thwaites_trans_cal/discovery_scripts/opt_script.csh .
ln -s $HOME/thwaites_trans_cal/discovery_scripts/add0upto3c .
ln -s $HOME/thwaites_trans_cal/discovery_scripts/clear_optim.sh .
...
if [ $2 == 'snap' ]; then
	...
	strtc=" STREAMICE_do_timedep_cost = .true."
	 ...
	 rm data.ctrl
        ...
        ln -s $HOME/thwaites_trans_cal/discovery_scripts/get_beta_bglen.py .
	if [ $1 == 'coul' ]; then
	
		python get_beta_bglen.py Coul 100 $ICESHARE
		strBeta=" STREAMICEbasaltracFile = 'BetaCoul.bin',"
		strBglen=" STREAMICEglenconstfile = 'BglenCoul.bin',"
        else
                python get_beta_bglen.py Weert 100 $ICESHARE
...
ln -s $HOME/thwaites_trans_cal/$build_dir/mitgcmuv_ad .
...
optimdir=OPTIM
builddir="$HOME/thwaites_trans_cal/optim_m1qn3/src"
...
cd $HOME/thwaites_trans_cal/$code_dir 
...
echo $builddir
cd $builddir
cp $HOME/thwaites_trans_cal/discovery_scripts/Makefile ./
str="                  -I$HOME/thwaites_trans_cal/$build_dir"
