function [metric_vel metric_acc metric_surf metric_dhdt T Vaf Tobs1 Tobs2 Tobs0 vafobs] = plot_val(do_plot, do_cal_plot)
...
grd = get_record(isRema,ad_folder,folder_start,1,5);
...
if (do_cal_plot);
	...
	uobs1(uobs1==-999999 | uobs1==0)=nan;
        ...
        u1 = get_record(isRema,ad_folder,folder_start,96,1); u1 = u1(I,J)';
        v1 = get_record(isRema,ad_folder,folder_start,96,2); v1 = v1(I,J)';
	c1 = sqrt(u1.^2+v1.^2);
	figure(h4);
...
for i = 1:length(nitervel)-1;
    ...
    u1 = get_record(isRema,ad_folder,folder_start,n1,1); u1 = u1(I,J)';
    u2 = get_record(isRema,ad_folder,folder_start,n2,1); u2 = u2(I,J)';
    v1 = get_record(isRema,ad_folder,folder_start,n1,2); v1 = v1(I,J)';
    v2 = get_record(isRema,ad_folder,folder_start,n2,2); v2 = v2(I,J)';
    th2 = get_record(isRema,ad_folder,folder_start,n2,3); th2 = th2(I,J)';
    th1 = get_record(isRema,ad_folder,folder_start,n1,3); th1 = th1(I,J)';
    

    mask1=get_record(isRema,ad_folder,folder_start,n1,4); mask1 = mask1(I,J)';
    mask2=get_record(isRema,ad_folder,folder_start,n2,4); mask2 = mask2(I,J)';
...
%	q = rdmds('land_ice',60);
%    q2 = rdmds('land_ice',660);
%elseif (niter0==96);
%	s96 = rdmds('H_streamiceinit')';
%	q = rdmds('land_ice',156);
%    q2 = rdmds('land_ice',756);
%elseif (niter0 == 0);
...
for i=1:3;
 ind = num2str(inds(i));
 eval(['hmask' ind '=transpose(get_record(isRema,ad_folder,folder_start,' ind ',4));']);
 eval(['h' ind '=transpose(get_record(isRema,ad_folder,folder_start,' ind ',3));']);
...
if (do_plot)
    ...
    %pcolor(x_mesh_mid/1e3,y_mesh_mid/1e3,(s156-sobs156)); shading flat; colorbar; caxis([-60 60]); colormap redblue; axis equal; axis tight
    %hold on
    ...
    %figure;
    %dhaf = haf2067-haf2017;
    %dhaf(mask_cost==0)=nan;
    %pcolor(x_mesh_mid,y_mesh_mid,dhaf); shading flat; colorbar; colormap parula; caxis([-500 0]);
...
for i = 3:3:756;
    ...
    if (~isRema | i>96);
	  ...
	  h = zeros(300,260);
          for j=0:2;
            h = h + 1/3 * get_record(isRema,ad_folder,folder_start,i-j,3)';
	  end
        else	
          h = get_record(isRema,ad_folder,folder_start,i,3)';
        end
        h = h(J,I);
        haf = h;
	haf(~vafmask)=0;
...
function vec = get_record(isRema,ad_folder,folder_start,n,rec);
