...
xlim([2000 2071])
ylabel('Vol above Floatation (Gt)')
...
hold off
for i=[1:4];
    times = .5*(time_st{i}(1:end-1)+time_st{i}(2:end));
    dvdt = diff(vaf_st{i})/1e9*4*.917;
    times = [times(1:32); times(33:end)];
    dvdt = [dvdt(1:32); dvdt(33:end)];


    h(i) = plot(2004+times,dvdt,'-o','markersize',5,'MarkerIndices',[40:40:756],'Color',colors{i},'MarkerFaceColor','auto','linewidth',2); hold on
end
for i=[1:4];
...
print -dpng vaf_dvaf_tseries.png
%close (figure(1));
figure(2);
