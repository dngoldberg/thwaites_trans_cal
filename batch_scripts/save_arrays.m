function [Bglen Beta rlow thick0 surf_a thick_a vx_a vy_a times] = get_arrays;
...
Bglen = rdmds(['B_glen_sqrt'])'.^2;
Beta = rdmds(['C_basal_fric'])'.^2;

if (isempty(ad_folder))
    thick0 = rdmds('H_streamiceinit')';
else
    thick0 = rdmds(['../' ad_folder '/runoptiter' appNum(niter,3) '/H_streamiceinit'])';
end
...
T = zeros(length(J),length(I));
surf_a = zeros(length(J),length(I),756/4);
thick_a = zeros(length(J),length(I),756/4);
vx_a = zeros(length(J),length(I),756/4);
vy_a = zeros(length(J),length(I),756/4);
    ...
    if (~isRema | i>96);
        ...
        if (i<=folder_start);
          ...
          for j=0:2;
            ...
            s = get_record(isRema,ad_folder,folder_start,i-j,6,read_issm);
            vx = get_record(isRema,ad_folder,folder_start,i-j,1,read_issm);
            vy = get_record(isRema,ad_folder,folder_start,i-j,2,read_issm);
	      end
        else
        	
          h = get_record(isRema,ad_folder,folder_start,i,3,read_issm);
          s = get_record(isRema,ad_folder,folder_start,i,6,read_issm);
          vx = get_record(isRema,ad_folder,folder_start,i,1,read_issm);
          vy = get_record(isRema,ad_folder,folder_start,i,2,read_issm);
        end
        %h = h(J,I);
        surf_a(:,:,k) = s;
        thick_a(:,:,k) = h;
