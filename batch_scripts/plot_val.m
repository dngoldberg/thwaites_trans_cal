function [metric_vel metric_vel_gr metric_acc metric_acc_trc metric_surf metric_dhdt dvafdt_err tikhbeta wgtvel wsu betacost bglencost priorcost thincost velcost T Vaf Tobs1 Tobs2 Tobs0 vafobs Bglen Beta] = ...
    plot_val(do_plot, do_cal_plot, morlighem_mat_file, period)
addpath('/home/dgoldber/network_links/datastore/ice_data/ThwaitesDataProphet/CODE');
addpath('/exports/geos.ed.ac.uk/iceocean/dgoldber/MITgcm_forinput/thwaites_trans_cal/batch_scripts');
...
while ~feof(fid)
    ...
    elseif contains(line,'tikhbeta');
        q=split(line,':'); 
		tikhbeta = str2num(strtrim(q{2}));
    elseif contains(line,'wgtvel');
        q=split(line,':'); 
		wgtvel = str2num(strtrim(q{2}));
    elseif contains(line,'wgtsurf');
        q=split(line,':'); 
		wsu = str2num(strtrim(q{2}));
    end
end
fclose(fid)
...
%     end
 ...
 GRD = THICK;
 RLOW = repmat(rlow,[1 1 size(THICK,3)]);
 GRD(RLOW<0) = GRD(RLOW<0) + 1027/917 * RLOW(RLOW<0);
 GRD(GRD<0)=0;
 GRD = (GRD>1);
...
nitervel2=[96 108 120 132 144 156];
nitersurf2=[156];

nitervel1=[24 36 48 60 72 84 96];
nitersurf1=[96];

if (period==1)
    nitersurf = nitersurf1;
    nitervel = nitervel1;
    year=2012
else
    nitersurf = nitersurf2;
    nitervel = nitervel2;
    year=2017
end
...
if (do_cal_plot);
	...
	uobs1(uobs1==-999999 | uobs1==0)=nan;
        ...
        u1 = get_record(isRema,ad_folder,folder_start,i,1,read_issm); 
        v1 = get_record(isRema,ad_folder,folder_start,i,2,read_issm); 
	c1 = sqrt(u1.^2+v1.^2);
	figure(h4);
    subplot(2,2,2-read_issm)
	pcolor(x_mesh_mid/1e3,y_mesh_mid/1e3,c1-cobs1); shading flat;  colormap(rgb2); 
	set(gca,'fontsize',14,'fontweight','bold');
	ylim([-600 -200]);
	
    if (~read_issm);
        h=colorbar('eastoutside'); title(h,'m/a');
        P = get(h,'position')
            set(h,'position',[P(1)+.06 P(2) P(3)*.6 P(4)-.04])
        colormap(h,rgb2)
    end
	xlabel('x (km)');
	ylabel('y (km)');
...
%     v2=q(I,J,2,2)';
    ...
    grd1=get_record(isRema,ad_folder,folder_start,n1,5,read_issm);
    grd2=get_record(isRema,ad_folder,folder_start,n2,5,read_issm);
    %c1(~f1)=nan;
    %c2(~f2)=nan;
    ...
    c2(grd2~=1) = nan;
    mis_speed_grd = (c2-cobs2).^2;

    dc(grd1==0 & ~isnan(dc))=nan;
    dc_obs(grd1==0 & ~isnan(dc_obs))=nan;

    mis = (dc-dc_obs).^2;

    dc_obs((abs(dc_obs)<100) & ~isnan(dc_obs))=nan;
    mis_trc = (dc-dc_obs).^2;
    

    %subplot(2,1,1)
    %pcolor(dc-dc_obs); shading flat; colorbar; caxis([-80 80]); colormap redblue; title(n2); 
    %subplot(2,1,2)
    %pcolor(c2-cobs2); shading flat; colorbar; caxis([-500 500]); colormap redblue; title(n2); drawnow; pause
    %if (i>1)
    mis_full = [mis_full mis(~isnan(mis))'];
    %end
    mis_speed_full = [mis_speed_full mis_speed(~isnan(mis_speed))'];
    mis_trc_full = [mis_trc_full mis_trc(~isnan(mis_trc))'];

    %mis_grd_full = [mis_grd_full ];
    mis_grd_speed_full=[mis_grd_speed_full mis_speed_grd(~isnan(mis_speed_grd))'];
    ...
    pcolor(x_mesh_mid,y_mesh_mid,dc-dc_obs(J,I)); shading flat; colorbar; caxis([-1500 1500]); axis equal; 
    hold on
    %h=q(I,J,3,2)';
    hmask=mask2;
...
metric_vel_gr = sqrt(mean(mis_grd_speed_full));
metric_acc = sqrt(mean(mis_full));
metric_acc_trc = sqrt(mean(mis_trc_full));
...
sobs156par = binread(['/home/dgoldber/network_links/geosIceOcean/dgoldber/MITgcm_forinput/thwaites_trans_cal/input_tc/surface_constraints/' filenamebase appNum(156,10) '.bin'],8,260,300)';
...
sobs156par(sobs156par==-999999)=nan;
sobs156=sobs156(J,I);
serr156=serr156(J,I);
sobs96=sobs96(J,I);
sobs36=sobs36(J,I);
sobs96par=sobs96par(J,I);
sobs156par=sobs156par(J,I);
sobs36par=sobs36par(J,I);
sobs156(~mask_cost)=nan;
serr96=serr96(J,I);
serr36=serr36(J,I);

gr_obs96=(-sobs96/(1-917/1027))<rlow;
gr_obs156=(-sobs156/(1-917/1027))<rlow;
...
if (period==2)
    ...
    mis_dhdt(haf156==0)=nan;
    mis_dhdt(~gr_obs156)=nan;
    metric_dhdt = sqrt(mean(mis_dhdt(~isnan(mis_dhdt))));
else
    mis_dhdt = (dhdt96 - dhdt_obs0).^2;
    mis_dhdt(haf96==0)=nan;
    mis_dhdt(~gr_obs96)=nan;
    metric_dhdt = sqrt(mean(mis_dhdt(~isnan(mis_dhdt))));
end
...
if (period==2)
    ...
    mis_surf = (s96 - sobs96).^2;
    mis_surf(hmask96~=1)=nan;
    mis_surf(isnan(sobs96))=nan;
    mis_surf(serr96~=1)=nan;
end
...
if (do_cal_plot)
    ...
    if (period==1)
        ...
        sdiff = s156-sobs156par;
        sdiff(haf156<=0)=nan;
        year=2017
    end
    length(sdiff(~isnan(sdiff)))
    pcolor(x_mesh_mid/1e3,y_mesh_mid/1e3,sdiff); shading flat;  axis tight; colormap redblue; 
        set(gca,'fontsize',14,'fontweight','bold');
        ylim([-600 -200]);
        if (~read_issm);
            h=colorbar('eastoutside'); title(h,'m');
            P = get(h,'position')
            set(h,'position',[P(1)+.06 P(2) P(3)*.6 P(4)])
            colormap(h,redblue)
        end
        ...
        freezeColors
        plot([x; x(1)]/1e3,[y; y(1)]/1e3,'k','linewidth',2)
        title(['surface misfit, ' num2str(year)])
	hold off
end;
...
dvafdt2012 = (Vaf(96/3)-Vaf(36/3))/5;
dvafdtobs = (vafobs(2)-vafobs(1))/5;
dvafdt_err = abs(dvafdt2012-dvafdtobs);
end    

[stat,str1]=system(['grep "fric smooth contr" ../' ad_folder '/outputoptiter' appNum(niter,3)]);
betacost = str2num(regexp(str1, '[\d\.\+\-D]+', 'match', 'once'));

[stat,str1]=system(['grep "bglen smooth contr" ../' ad_folder '/outputoptiter' appNum(niter,3)]);
bglencost = str2num(regexp(str1, '[\d\.\+\-D]+', 'match', 'once'));

[stat,str1]=system(['grep "prior smooth contr" ../' ad_folder '/outputoptiter' appNum(niter,3)]);
priorcost = str2num(regexp(str1, '[\d\.\+\-D]+', 'match', 'once'));

[stat,str1]=system(['grep "thinning contr" ../' ad_folder '/outputoptiter' appNum(niter,3)]);
thincost = str2num(regexp(str1, '[\d\.\+\-D]+', 'match', 'once'));

[stat,str1]=system(['grep "td vel misfit" ../' ad_folder '/outputoptiter' appNum(niter,3)]);
velcost = str2num(regexp(str1, '[\d\.\+\-D]+', 'match', 'once'));


% fric smooth contr =  0.219592214486275D+00
% bglen smooth contr =  0.111799682967837D+00
% prior smooth contr =  0.406555248969412D+00
% thinning contr =  0.149015279808751D+03
% td vel misfit =  0.000000000000000D+00
